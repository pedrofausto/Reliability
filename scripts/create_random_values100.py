...
def netlistCopy(vdd, temp, act, period, age, netlist_name):
  ...
  bashCommand="cp "+"inv100"+" " + netlist_name
  #print bashCommand
...
for j in range(0,400):
  ...
  if (ageFlag == False):
    netlist = "inv100vdd11t27a80p40us_ag24h"
    ageFlag = True
    ...
    netlist = "inv100vdd" + str(randVdd).replace(".","") + "t" + str(randTemp) + "a80p40us_ag"+ str(age).replace(".","_") + "h"
    netlistCopy(randVdd,randTemp,"80","40",age,netlist)
    age = age + 24.0
    netlists.append(netlist)

    netlist = "inv100vdd11t27a80p40us_ag"+ str(age).replace(".","_") + "h"
    netlistCopy(1.1,27,"80","40",age,netlist)
    age = 24.0
    netlists.append(netlist)
    ageFlag = False
...
flag = 0
count = 1;
extractFile.write('out_delay=outfile("./delays_inv100", "a")\n')
extractFile.write('fprintf(out_delay "Vdd;Temp;DelayRising;DelayFalling\\n")\n')
count = 0
for net in netlists:
  if flag == 0:
    firstNetlist = net
    ...
    # Printing both values: Rising and Falling
...
if (result != 0):
 ...
 quit()
  ...
  bashCommand="rm -Rf inv100vdd* resultados*"
  #os.system(bashCommand)
